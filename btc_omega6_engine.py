...
def backtest(
    ...
    # Position state — clean single source of truth
    pos_dir   = 0       # +1 long, -1 short, 0 flat
    avg_ep    = 0.0     # average entry price
    total_u   = 0.0     # total units (BTC for long, USD-notional for short)
    total_c   = 0.0     # total USD deployed
    stop      = 0.0
    tp1       = 0.0     # partial target
    tp2       = 0.0     # full target
    hw        = 0.0     # high-water for trailing stop
    lw        = 0.0     # low-water for short trailing stop
    partial_x = False
    pyramid_x = False
    entry_bar = 0
    ...
    for i in range(1, n):
        ...
        if pos_dir != 0:
            ...
            if pos_dir == 1:
                ...
                if l_arr[i] <= stop:
                    exit_px, exit_r = stop, "STOP"
                elif partial_exit and not partial_x and h_arr[i] >= tp1:
                    # Partial exit: close 50% at tp1
                    close_u   = total_u * 0.50
                    proceeds  = close_u * tp1 * (1 - fee_pct)
                    cost_half = total_c * 0.50
                    pnl_half  = proceeds - cost_half
                    capital  += proceeds
                    total_u  -= close_u
                    total_c  -= cost_half
                    partial_x = True
                    stop      = max(stop, avg_ep * 1.001)  # move stop to break-even
                    daily[day] = daily.get(day,0.0) + pnl_half
                    equity.append(capital + total_u * px)
                    continue
                elif pyramid and not pyramid_x and not partial_x and px >= avg_ep*(1+sl_pct/100):
                    # Pyramid: add 25% more
                    add_usd = kelly.size(capital, 1, RegimeDetector.LONG_MULT.get(reg,0.5)) * 0.25
                    add_usd = min(add_usd, capital * 0.04)
                    if capital > add_usd + 200:
                    ...
                    exit_px, exit_r = tp2, "TP"
                elif cv <= -threshold and reg in [1,3]:
                    exit_px, exit_r = px, "SIGNAL"
                elif i - entry_bar >= max_bars:
                    exit_px, exit_r = px, "TIME"
            else:
                ...
                if h_arr[i] >= stop:
                    exit_px, exit_r = stop, "STOP"
                elif partial_exit and not partial_x and l_arr[i] <= tp1:
                    close_u   = total_u * 0.50
                    short_pnl = (avg_ep - tp1) * close_u * (1 - fee_pct)
                    capital  += short_pnl + (close_u * avg_ep * 0.50)  # return margin
                    total_u  -= close_u
                    total_c  *= 0.50
                    partial_x = True
                    stop      = min(stop, avg_ep * 0.999)
                    ...
                    exit_px, exit_r = tp2, "TP"
                elif cv >= threshold and reg in [0,2]:
                    exit_px, exit_r = px, "SIGNAL"
                elif i - entry_bar >= max_bars:
                    exit_px, exit_r = px, "TIME"
            ...
            if exit_px:
                if pos_dir == 1:
                    ...
                    pnl      = (avg_ep - exit_px) * total_u * (1 - fee_pct)
                    proceeds = total_c + pnl   # return margin + profit
                ...
                pos_dir=0; avg_ep=0; total_u=0; total_c=0
                partial_x=False; pyramid_x=False
            ...
            # LONG entry
            if cv >= threshold and lm > 0 and vol_ok[i] and capital > 300:
                trade_usd = kelly.size(capital, 1, lm)
                trade_usd = min(trade_usd, capital * 0.10)
                net_u     = trade_usd*(1-fee_pct)/px
                capital  -= trade_usd
                pos_dir=1; avg_ep=px; total_u=net_u; total_c=trade_usd
                stop=px*(1-sl_pct/100); tp1=px*(1+sl_pct*1.5/100); tp2=px*(1+tp_pct/100)
                hw=px; entry_bar=i; partial_x=False; pyramid_x=False

            # SHORT entry
            elif cv <= -threshold and sm > 0 and vol_ok[i] and capital > 300:
                trade_usd = kelly.size(capital, -1, sm)
                trade_usd = min(trade_usd, capital * 0.10)
                btc_u     = trade_usd / px  # BTC shorted
                capital  -= trade_usd * 0.20  # 20% margin reserved
                pos_dir=-1; avg_ep=px; total_u=btc_u; total_c=trade_usd
                stop=px*(1+sl_pct/100); tp1=px*(1-sl_pct*1.5/100); tp2=px*(1-tp_pct/100)
                lw=px; entry_bar=i; partial_x=False; pyramid_x=False
    ...
    if pos_dir != 0:
        ...
        trades.append({
            "entry":avg_ep,"exit":px,"direction":"LONG" if pos_dir==1 else "SHORT",
            "regime":0,"regime_name":"END","pnl_usd":pnl,
            "pnl_pct":pnl/(total_c+1e-9)*100,"reason":"END","size_usd":total_c,"bars":n-1-entry_bar,
        })
        equity[-1] = capital

    elapsed = max((df["timestamp"].iloc[-1] - df["timestamp"].iloc[0]).days, 1)
    return _stats(trades, equity, daily, capital, start_cap, elapsed)
